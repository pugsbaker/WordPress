...
function wppl_thumbnail($wppl, $post) {
	if( isset($wppl['show_thumb']) ) {
		echo '<div class="wppl-thumb">';
			...
			//echo '<a href="'.$link[1] .'" class="thickbox">' . get_the_post_thumbnail($post->ID) . '</a>';	
		echo '</div>';
	}	
}
...
function wppl_additional_info($wppl, $post) { ?>
	<?php if( isset($wppl['additional_info']) ); { ?>
		<div class="wppl-additional-info">	
   		<?php if( isset($wppl['additional_info']['phone']) &&  (get_post_meta($post->ID,'_wppl_phone',true))) { ?> <div class="wppl-phone"><span>Phone: </span><?php echo (get_post_meta($post->ID,'_wppl_phone',true)) ? get_post_meta($post->ID,'_wppl_phone',true) : 'N/A'; ?></div><?php } ?>
    		...
    		<?php if( isset($wppl['additional_info']['website']) && (get_post_meta($post->ID,'_wppl_website',true)) ) { ?><div class="wppl-website"><span>Website: </span><?php if (get_post_meta($post->ID,'_wppl_website',true)) { ?><a href="http://<?php echo get_post_meta($post->ID,_wppl_website,true); ?>" target="_blank"><?php echo get_post_meta($post->ID,'_wppl_website',true); } else { echo "N/A"; }?></a></div><?php } ?>
		</div> <!-- additional info -->
	<?php } ?>
<?php } ?>
<?php
